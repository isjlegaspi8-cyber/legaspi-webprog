    ...
    <div className="flex w-full flex-col gap-24 pb-20 bg-zinc-950">
      ...
      <section className="relative min-h-[85vh] flex items-center border-b-[12px] border-zinc-900 bg-zinc-950 px-6 py-24 md:px-12 md:py-40 text-white overflow-hidden">
        ...
        <div className="relative z-20 w-full">
          ...
          {/* FIXED H1: Forced white text and cleared background-clip from index.css */}
          <h1 
            className="max-w-6xl text-6xl md:text-[10rem] font-black italic uppercase leading-[0.9] tracking-tighter mb-16 !text-white"
            style={{ WebkitTextFillColor: 'white', background: 'none' }}
          >
            PENDRAGON <br /> 
            <span className="!text-yellow-500 font-black" style={{ WebkitTextFillColor: '#eab308' }}>DOMINATION.</span>
          </h1>

          <div className="grid md:grid-cols-2 gap-12 items-start">
            <p className="max-w-xl text-lg md:text-2xl font-bold !text-zinc-100 leading-snug border-l-8 border-yellow-500 pl-8 italic">
      ...
      <section className="px-6 md:px-12 relative z-30 -mt-12">
        <div className="grid gap-8 sm:grid-cols-3">
          {[
            { label: "Current Rank", val: "#01", theme: "bg-zinc-100 !text-black" },
            { label: "Win Streak", val: "UNBROKEN", theme: "bg-yellow-500 !text-black" },
            { label: "Gear Tier", val: "APEX", theme: "bg-zinc-100 !text-black" }
          ].map((stat, i) => (
      ...
      <section className="px-6 md:px-12 mt-12">
        ...
        <div className="grid gap-12 md:grid-cols-3">
          {[
            ...
            <article key={i} className="group relative rounded-[3rem] border-4 border-zinc-900 bg-zinc-900 p-8 shadow-[15px_15px_0_0_#eab308] hover:translate-y-[-15px] transition-all duration-500">
              ...
              <div className="mt-10">
                <p className="text-xs font-black text-yellow-500 uppercase tracking-widest italic mb-2">{member.role}</p>
                <h3 className="text-4xl font-black italic uppercase !text-white leading-tight">{member.name}</h3>
                <div className="mt-8 h-2.5 w-full bg-zinc-950 rounded-full overflow-hidden border-2 border-zinc-800">
                  <div className="h-full bg-zinc-800 w-[100%] group-hover:bg-yellow-500 transition-all duration-700"></div>
                </div>
              </div>
            </article>
          ))}
