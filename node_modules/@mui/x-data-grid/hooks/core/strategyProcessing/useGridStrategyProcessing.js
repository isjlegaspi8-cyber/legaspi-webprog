...
const useGridStrategyProcessing = apiRef => {
  ...
  const applyStrategyProcessor = React.useCallback((processorName, params) => {
    ...
    if (activeStrategy == null) {
      throw new Error(process.env.NODE_ENV !== "production" ? "MUI X: Can't apply a strategy processor before defining an active strategy" : (0, _formatErrorMessage2.default)(72));
    }
    const groupCache = strategiesCache.current[processorName];
    if (!groupCache || !groupCache[activeStrategy]) {
      throw new Error(process.env.NODE_ENV !== "production" ? `MUI X: No processor found for processor "${processorName}" on strategy "${activeStrategy}"` : (0, _formatErrorMessage2.default)(73, processorName, activeStrategy));
    }
